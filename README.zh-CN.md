...
定义在 `config/sources.json`（8 个来源）：

- Amazon SP-API Release Notes（官方）
- Amazon Ads Library（官方）
- Helium 10 博客（行业）
- Serious Sellers Podcast — RSS（媒体）
- AMZ123 跨境头条 — `/t` 链接列表（行业）
- WeAreSellers — RSS / HTML（社区）
- Billion Dollar Sellers — 归档（newsletter）
- My Amazon Guy — YouTube RSS（媒体）

每个来源有 `lookbackHours` 时间窗；超窗的、以及上一次已推送过的（按
`state-feed.json`，每个安装本地维护）都会被过滤掉，日报只含新增。
...
- `thin`（可选，来源只返回索引/JS 壳时为 `true`——agent 如实说明，不编造摘要）
