...
#   1. runs prepare-digest.js (writes data/feed-amazon.json + state, emits blob)
#   2. has Claude Code (headless) remix the blob into digest/<date>.md
#   3. if the Claude CLI isn't available, writes a clearly-labelled RAW digest
#      so delivery still produces something honest
...
if command -v claude >/dev/null 2>&1; then
  log "claude CLI found — remixing via agent"
  claude -p "Run the follow-amazon-daily daily workflow. Read $BLOB (already fetched — do NOT fetch anything yourself). Follow prompts/daily-digest.md, the prompts/summarize-*.md sub-prompts, and prompts/translate.md, honoring blob.config.language. Write the finished editorial digest to $DIGEST. Output nothing else." >>"$LOG" 2>&1 \
    && [ -s "$DIGEST" ] && REMIXED=1
fi

if [ "$REMIXED" -ne 1 ]; then
  log "no agent remix — writing labelled RAW fallback digest"
  node -e '
    const fs=require("fs");
    ...
    L.push("");
    L.push("> ⚠️ RAW feed — NOT editorially remixed. The Claude CLI was unavailable on this");
    L.push("> cron run, so this is the structured feed without agent summaries.");
    L.push("> Run the follow-amazon-daily skill manually for the full digest.");
    L.push("");
    for(const c of cats){
...
if node scripts/deliver.js --file "$DIGEST" >>"$LOG" 2>&1; then
  ...
  log "delivery failed for $DIGEST"
  exit 1
fi
